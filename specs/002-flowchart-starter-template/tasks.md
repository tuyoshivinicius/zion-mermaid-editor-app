...
- [X] T001 Install `@radix-ui/react-alert-dialog` as a runtime dependency in `package.json` (`npm install @radix-ui/react-alert-dialog`) — the single new dependency of the slice, extending the shadcn/ui layer already fixed by ADR-004 (Decisão I)
- [X] T002 Generate the shadcn/ui alert-dialog primitive at `src/components/ui/alert-dialog.tsx` via `npx shadcn@latest add alert-dialog` — new file; do not modify `src/components/ui/button.tsx` or any existing primitive
- [X] T003 [P] Add the `test:e2e:perf` script (`playwright test --config playwright.perf.config.ts`) to `package.json`, leaving the existing `test`, `test:e2e` and `test:perf` scripts untouched
- [X] T004 [P] Create `playwright.perf.config.ts` at the repository root: `testDir: './tests/e2e'`, `testMatch: '**/*.perf.ts'`, single `chromium` project, `baseURL: 'http://localhost:4173'`, `webServer.command: 'npm run build && npm run preview -- --port 4173'` (Decisão J, FR-018). **MUST NOT** touch `playwright.config.ts` — in particular, do not add `testIgnore` to it; its default `testMatch` already excludes `*.perf.ts`

- [X] T004a [P] Create `tests/helpers/percentile.ts` exporting `percentile(sorted: number[], p: number): number` — `sorted[Math.min(sorted.length - 1, Math.floor(sorted.length * p))]`, a **verbatim** copy of the formula S0 uses at `tests/perf/preview-latency.test.ts:20`, taking a **pre-sorted** array (the caller sorts). It is copied, not imported: S0's is module-private and lives inside a Vitest test module with a top-level `describe`, which a Playwright spec cannot import without executing the suite. Copying keeps `tests/perf/` untouched (T030) and FR-014a's two-amendment list exhaustive. **New file — do not touch `tests/perf/`**
...
US2 compares against its canonical text, US3 asserts its absence. The FR-012 gate (T008a) lands here too, so
it guards all three stories while they are being written rather than being checked once at the end.

**⚠️ CRITICAL**: No user story work can begin until this phase is complete.

- [X] T005 [P] Write the failing unit test `tests/unit/starter-model.test.ts` covering the starter contract (ST1–ST5): exact shape (5 nodes / 5 edges / `direction: 'TD'` / pt-BR labels / `aprovado` has `shape: 'diamond'` — the model half of FR-002a, whose code half is `aprovado{Aprovado?}` inside the documented text below, and whose canvas half T011 asserts); `node.id === deriveSlug(node.label)` asserted against `src/core/slug/` (**not** hand-copied); unique ids and `e{index}` edge-id convention; every `edge.source`/`edge.target` resolves to an existing node; `STARTER_TEXT` equals the documented 11 lines; `STARTER_MODEL.nodes.length <= 6` and `STARTER_TEXT.trimEnd().split('\n').length <= 14`; and the FR-015 round-trip `await importFlowchart(STARTER_TEXT)` returns `ok: true` and preserves structural content. **The round-trip comparison MUST accommodate the two verified reimport normalizations** — treat `TD ≡ TB`, and compare edges by `(source, target, connector, label)` **excluding the id** (the ACL returns `e0-inicio-revisar`). An `expect(reimported).toEqual(STARTER_MODEL)` goes red on correct code; do not "fix" the starter to make it green (ST4)
- [X] T006 Create `src/starter/model.ts` exporting `STARTER_MODEL: GraphModel` (deep-frozen; the exact constant in `data-model.md` §1) and `STARTER_TEXT = generate(STARTER_MODEL)`. **No Mermaid string of the starter may exist in source** — the text is derived, never hand-written (ST3), which is what makes the canonical form true by construction rather than by coincidence. Makes T005 pass
- [X] T007 [P] Add the S1 pt-BR strings to `src/strings.ts` (clear button label, dialog title/description/confirm/cancel, `starter.seeded` announcement, clear-completed announcement) — additive only; no existing string changes
- [X] T008 [P] Amend `specs/001-s0-walking-skeleton/contracts/generator.contract.md` with **G9 — Forma de emissão**, ratifying the own-line emission the S0 generator already implements (FR-003a). Documentation of existing behaviour: **zero** code and **zero** test changes in S0 follow from this
- [X] T008a [P] Write `tests/contract/no-template-selector.test.ts` — the FR-012/SC-011 gate, following the **existing S0 contract-test idiom** (walk `src/` + `package.json`, collect offenders, `expect(offenders).toEqual([])`; see `tests/contract/no-image-export.test.ts`). Two assertions: (1) **no selection surface** — no source file matches a template-*choice* pattern (`/template\s*(selector|picker|gallery|chooser|list)/i`, `/(escolher|selecionar|choose|select)\s+(um\s+)?template/i`, a `role="listbox"|"combobox"|"menu"` bound to templates). Match the **choice**, not the word "template": the slice is *named* "template starter", so `src/starter/` and every comment mentioning a template must pass; (2) **the starter is singular** — `import * as m from '@/starter/model'` exports exactly `['STARTER_MODEL', 'STARTER_TEXT']`, `STARTER_MODEL` is not an array, and no registry/collection export (`TEMPLATES`, `STARTER_TEMPLATES`) exists. A gallery needs a *plural* to draw from; forbidding the plural is what makes FR-012 decidable rather than a matter of taste. **New file in `tests/contract/` — the four S0 gates there are untouched** (additive, per Decisão F)

**Checkpoint**: `npm test -- tests/unit/starter-model.test.ts tests/contract/no-template-selector.test.ts` is green; the starter is proven valid, canonical, within both ceilings, and singular — with no surface to choose it from.
...
- [X] T009 [P] [US1] Write `tests/unit/starter-seed.test.ts` for the pure seed decision (SD1/SD2, SC-008): `shouldSeedStarter({ restorableDraft: null })` is `true`; it is `false` for **any** non-null draft — **including one representing an empty canvas** (`{ text: '' }`), since the mere existence of restorable content marks a returning user. Pass a **simulated** draft: nothing of the RN-06 buffer is built in this slice (FR-011)
- [X] T010 [P] [US1] Write `tests/unit/starter-announce.test.tsx` (RTL) asserting SC-013 as a **transition**: the status region does **not** contain the seeding message on first paint, and does contain it afterwards. A test that only asserts the final text passes equally over a mute implementation (a live region born filled is never spoken) — which is exactly the failure this test exists to exclude (FR-017a, Decisão I-bis). Note the seeding flag behind `starterWasSeeded()` is **module state** (SD7) and survives between cases in the same file: drive `seedStarter` explicitly per case, or `vi.resetModules()` between them — do not let import order decide the result
- [X] T011 [P] [US1] Write `tests/e2e/starter.spec.ts` for US1 (SC-001/002/003/004/005/009/011 · FR-002a) at viewport 1280×800: starter visible on the canvas and in `code-input` with zero actions; code byte-identical to the generated canonical form; each of the five core S0 actions rewrites the code; copy yields a valid Flowchart; the whole diagram is visible without zoom or pan. Also assert **FR-002a — the deliberate shape divergence**, in both directions: the code panel shows the decision as a diamond (`aprovado{Aprovado?}`, already implied by the byte-identical canonical form above), while on the canvas the `Aprovado?` node is drawn by the **same renderer as every other node**. Assert this as **sameness**, not as "is a rectangle": `page.getByLabel('Aprovado?')` and `page.getByLabel('Início')` carry the **identical** class attribute, and no diamond marker (`clip-path`, `rotate`, `<polygon>`) exists on the canvas. Sameness is what FR-002a/FR-004 actually promise — the starter gets no special treatment — and it is what `FlowNode` delivers today (one `rounded-md border …` div per node; `shape` is never passed from `CanvasPanel`). Without this, a future change that renders the diamond would pass every other test while silently contradicting FR-002a and the canvas↔code parity this slice scopes to **nodes, edges and labels only**. Also assert **SC-011** at the surface the user actually meets: at first contact no template-choice control is presented — the toolbar's interactive controls are exactly the known allowlist (`new-node-label`, `add-node-button`, `connect-mode-button`, plus S1's clear control), and no `listbox`/`combobox`/`menu` role offering a template exists anywhere on the page. T008a gates the source; this gates the UI. For SC-003, assert **absence of gratuitous rewriting** — lines untouched by the edit keep exact text and order — and **not** a fixed-size diff: renaming `Revisar` legitimately rewrites 4 of the 11 lines via the S0 id cascade (FR-003), and that satisfies SC-003 rather than violating it
- [X] T012 [P] [US1] Write `tests/e2e/starter-boot-latency.perf.ts` for the SC-012 gate: ≥1 warm-up load, then **30** `page.goto('/')` samples, clock starting before the goto and stopping when the starter nodes are visible on the canvas **AND** the text is present in the code panel; sort the samples ascending and take the p95 via `tests/helpers/percentile.ts` (T004a) — the **same formula** S0 uses, deliberately copied rather than imported (see T004a); assert p95 ≤ 1000 ms. At N=30, `floor(30 × 0.95) = 28` selects the 29th of 30 samples, which is why FR-018 fixes 30 as the smallest sample size yielding a real p95 instead of the literal maximum. Runs only under `playwright.perf.config.ts` (production build) — the `.perf.ts` suffix keeps it out of the S0 config's default `testMatch`

### Implementation for User Story 1

- [X] T013 [US1] Create `src/starter/seed.ts` exporting `RestorableDraft`, `SeedInput`, the pure `shouldSeedStarter(input)` (`input.restorableDraft === null` — reads no store, no `window`, no storage, no clock) and `seedStarter(input): boolean`, which on seeding applies a **single** `useEditorStore.setState({ model: STARTER_MODEL, lastValidModel: STARTER_MODEL, editorText: STARTER_TEXT })` (SD1–SD4). Also export `starterWasSeeded(): boolean`, reading a module-private flag that `seedStarter` writes exactly once (SD7): it is the channel `<StarterAnnouncer />` (T015) uses to know whether to announce, since `main.tsx` runs before the render and cannot hand the return value to a component mounted after it without changing `App`'s signature (FR-014). The flag takes **no part** in the seeding decision and is not a session marker (SD1/SD5). Makes T009 pass
- [X] T014 [US1] In `src/main.tsx`, import and call `seedStarter({ restorableDraft: null })` **synchronously before** `createRoot(...).render(<App />)` — a literal `null`, since this slice introduces no real producer of restorable content (SD3). Not from a `useEffect`: the first paint must already contain the starter, so S0's empty state is **never rendered** — not "empty for an instant", but empty never shown (FR-001a/SC-001). Do not change the store's initial state, which remains `createEmptyModel()` and is literally the clear's target (SD6, Decisão G)
- [X] T015 [P] [US1] Create `src/components/StarterAnnouncer.tsx`: a mount `useEffect` calling the `announce()` S0 already exposes with `strings.starter.seeded`, **only** when `starterWasSeeded()` (T013/SD7) is true. Post-mount so the live region **transitions** from empty to the message (FR-017a). Makes T010 pass
- [X] T016 [US1] Mount `<StarterAnnouncer />` in `src/App.tsx` — additive; `StatusRegion` and the existing layout are unchanged

### Authorized S0 fixture amendments (FR-014a — exactly two, exhaustive)

- [X] T017 [P] [US1] Amend the reload assertion in `tests/e2e/ephemeral.spec.ts`: `toHaveValue('')` becomes an assertion of what the test actually exists to prove — that the **user's work** does not survive the reload and the panel returns to the starter's canonical form, a fresh first contact. The `storageLength === 0` / no-save-affordance assertions stay **exactly** as they are: the starter preserves nothing and overwrites nothing (FR-010)
- [X] T018 [P] [US1] Repoint **and rename** the `invalid text as the very first input` case in `tests/e2e/us1-preview.spec.ts`: with the starter seeded, `lastValidModel` is the starter, so the last valid preview is no longer the empty state. Today it only passes because it asserts the status indicator, not the state its name refers to — leaving it green while asserting something false
...
- [X] T020 [P] [US2] Write `tests/unit/clear-dialog.test.tsx` (RTL) for FR-016/CL6: focus enters the dialog on open, `Escape` cancels, focus **returns to the clear control** on close (both confirming and cancelling), the dialog has an accessible name and role, and cancelling leaves content exactly as it was
...
- [X] T029 Run the **Decisão F diff gate**: `git diff --stat main -- src/` MUST show only the six additive S0 files (`main.tsx`, `App.tsx`, `strings.ts`, `components/Toolbar.tsx`, `components/CanvasPanel.tsx`, `state/editorStore.ts`) plus S1's new files (`starter/*`, `components/ClearAction.tsx`, `components/StarterAnnouncer.tsx`, `components/ui/alert-dialog.tsx`). **`src/core/**` must be absent** — which alone is what keeps constitution gates II, IV, V and VI green by construction. Any other S0 file in the diff blocks the merge (FR-014)
- [X] T030 [P] Run `npm test` and confirm the whole Vitest suite is green — in particular that S0's five contract gates (`acl-isolation`, `import-acl`, `no-coordinates`, `no-image-export`, `no-nextjs`), `tests/roundtrip/` and `tests/perf/` are **unchanged and still passing**, and that S1's new `tests/contract/no-template-selector.test.ts` (T008a, FR-012/SC-011) is green. Adding a file to `tests/contract/` is additive and does not touch the S0 gates (Decisão F); `tests/perf/` in particular stays untouched because the SC-012 gate copies the percentile formula rather than importing it (T004a)
- [X] T031 [P] Run `npm run test:e2e` and confirm all three browsers are green against the **unchanged** `playwright.config.ts` (FR-018), with only the two FR-014a amendments (T017, T018) in the S0 e2e diff
- [X] T032 Run `npm run test:e2e:perf` and confirm the SC-012 gate is green: p95 ≤ 1 s over 30 samples against the production build. **If the p95 grazes the ceiling on an idle runner**, the thing to revisit is the 1 s ceiling — not the instrument. Measuring the wrong artifact to get green defeats the gate's purpose
- [X] T033 [P] Run `npm run lint` and confirm the `no-restricted-imports` rule is green — S1 imports `mermaid` **nowhere** (gate VI/ADR-002: seeding does not traverse the parse path)
- [X] T034 Walk the `quickstart.md` completion checklist end-to-end, opening the app with `npm run dev`. Watch specifically for a **flash of the empty state** before the starter: that is an FR-001a bug, not a performance detail — check that `seedStarter(...)` runs before `createRoot(...).render(<App />)` and not from a `useEffect`
...
- T003, T004 and T004a in parallel (different files)
- T005, T007, T008, T008a in parallel (test, strings, S0 docs, FR-012 gate — no overlap)
