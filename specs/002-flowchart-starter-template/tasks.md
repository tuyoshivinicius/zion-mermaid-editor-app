...
- [ ] T015 [P] [US1] Create `src/components/StarterAnnouncer.tsx`: a mount `useEffect` calling the `announce()` S0 already exposes with `strings.starter.seeded`, **only** when `starterWasSeeded()` (T013/SD7) is true. Post-mount so the live region **transitions** from empty to the message (FR-017a). Makes T010 pass
